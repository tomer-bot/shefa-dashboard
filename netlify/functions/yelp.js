...
if(path.startsWith('budget/')){
  ...
  if (path === 'reporting' || path.startsWith('reporting/')) {
    ...
    for (let i = 0; i < bizIds.length; i += batchSize) {
      const batch = bizIds.slice(i, i+batchSize);
      const postData = JSON.stringify({ ids: batch, start: startDate, end: endDate, metrics: ["num_calls","num_mobile_cta_clicks","num_desktop_cta_clicks","num_mobile_search_appearances","url_clicks","num_messages_to_business","num_total_page_views","num_user_photos","num_check_ins"] });
